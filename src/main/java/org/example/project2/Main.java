...
class Main {
    ...
    private static Stack<Plate> plateStack = new Stack<>();
    private static Stack<ZincPlate> zincPlateStack = new Stack<>();
    ...
    public static void menu(BufferedReader br) throws IOException {
        ...
        while (true) {
            ...
            if(diameter <= 0) {System.out.println("Please enter again"); continue;}
    ...
    public static void sortPlateStack(){
        ...
        while(!plateStack.empty()){
            ...
            while (!tempStack.empty() && tempStack.peek().getDiameter() > t){
    ...
    public static void printPlates(){
        ...
        for (Plate plate : plateStack) {
            System.out.println(plate.getClass().getSimpleName() + " " + plate.getDiameter());
        }
        System.out.println();

        System.out.println("Zinc Plates");
        for (ZincPlate zincPlate : zincPlateStack) {
            System.out.println(zincPlate.getClass().getSimpleName() + " " + zincPlate.getDiameter());
        }
    }
}
